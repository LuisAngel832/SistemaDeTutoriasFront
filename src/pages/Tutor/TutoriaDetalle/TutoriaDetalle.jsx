...
const TutoriaDetalleTutor = () => {
  ...
  return (
    <main className="tdt-main">
      <button type="button" className="tdt-back" onClick={() => navigate(ROUTES.tutor.inicio)}>
        ← Mis tutorias
      </button>
      ...
      {isLoading ? (
        ...
        <div className="tdt-error">
          {error}
          <Link to={ROUTES.tutor.inicio} className="tdt-error-back">
            Volver a mis tutorias
          </Link>
        </div>
      ) : tutoria ? (
        <>
          {feedback ? <div className={`tdt-feedback ${feedback.type}`}>{feedback.text}</div> : null}
          ...
          <section className="tdt-comentarios-section">
            ...
            <p className="tdt-comentarios-sub">Sugerencias y observaciones previas a la sesion.</p>
